...
from transforms.postprocessing import TessOCRPostprocessor
...
def main(output_path, input_path, verbose):
    ...
    ocr = PyTesseractOCR()
    postprocessor = TessOCRPostprocessor()
    pipe = None
    ...
    if write_lines_to_file(data, output_path):
        logger.info("Writing to the output file...")
    else:
        logger.error("Writing to the output file crashed...")
